...
function findLongestWord(arr) {
  ...
  for (let i = 0; i < arr.length; i++) {
    if (checkLong.length < arr[i].length) {
      checkLong = arr[i];
    }
  }
  return checkLong;
...
function sum(arr) {
  ...
  console.log(`resultado dentro de la funcion ${result}`);
  return result;
}
...
function averageWordLength(arrWords) {
  ...
  for (i = 0; i < arrWords.length; i++) {
    let index = arrWords[i];
    sumLong += index.length;
  }
  return sumLong / arrWords.length;
}
